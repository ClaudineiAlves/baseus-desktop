...
fn command_bytes(arg: &str) -> Option<Vec<u8>> {
    Some(match arg {
        ...
        "q:01" => vec![0xBA, 0x01],
        "q:02" => vec![0xBA, 0x02],
        "anc:lvl40" => vec![0xBA, 0x34, 0x01, 0x40],
        "anc:lvl80" => vec![0xBA, 0x34, 0x01, 0x80],
        "gest:set" => vec![0xBA, 0x8D, 0x00, 0x00, 0x04],
        "gest:commit0" => vec![0xBA, 0x8E, 0x00, 0x00, 0x06],
        "gq:left" => vec![0xBA, 0x8C, 0x00, 0xFF],
        "gq:right" => vec![0xBA, 0x8C, 0x01, 0xFF],
        "gq:8a" => vec![0xBA, 0x8A],
        "gq:8b" => vec![0xBA, 0x8B],
        _ => return None,
    })
