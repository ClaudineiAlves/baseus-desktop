...
fn command_bytes(arg: &str) -> Option<Vec<u8>> {
    Some(match arg {
        ...
        "eq:classic" => baseus_protocol::types::EqMode::BaseusClassic
            .frame()
            .to_vec(),
        // Gesture: BA 8D <side> <key> <func>. Confirmed keys: Double=03 Triple=00 LongPress=01 TapHold=02.
        "gest:double-next" => vec![0xBA, 0x8D, 0x00, 0x03, 0x01],
        "gest:triple-volup" => vec![0xBA, 0x8D, 0x00, 0x00, 0x0B],
        "gest:long-prev" => vec![0xBA, 0x8D, 0x00, 0x01, 0x02],
